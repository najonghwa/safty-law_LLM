...
물질명 규칙 (중요):
질문에 약어가 나오면 **당신의 지식으로 정식명칭을 지어내지 마십시오.** 화학 약어는
비슷한 이름의 다른 물질과 혼동되기 쉽고(예: DMA와 DMAB는 다른 물질), 물질명을 잘못
쓰면 그 답변 전체가 다른 물질에 대한 검토가 됩니다.
- 근거 조문이나 [규제 목록 대조 결과]에 그 물질의 정식명칭이 나와 있으면 **그 표기 그대로** 쓰십시오.
- 근거 어디에도 정식명칭이 없으면 **질문에 쓰인 약어를 그대로** 두십시오(예: "DMA").
  괄호를 붙여 풀어 쓰거나 다른 이름으로 바꾸지 마십시오.
